...
class UserAgentPoolUnitTest(unittest.TestCase):
    ...
    def test_get_random_ua_returns_valid_string(self):
        """
        测试获取随机User-Agent返回有效字符串的功能

        验证UserAgentPool.get_random_ua()方法能够返回有效的User-Agent字符串，
        确保返回值为字符串类型、非空且包含'Mozilla'关键字。
        """
...
class SmartSchedulerUnitTest(unittest.TestCase):
    ...
    def test_record_success(self):
        ...
        self.assertIn(url, self.scheduler.url_status)
        self.assertEqual(self.scheduler.url_status[url]['status'], 'success')
    ...
    def test_response_time_based_adjustment(self):
        ...
        self.assertGreater(delay_after_slow, delay_after_fast)
...
class IncrementalCrawlerUnitTest(unittest.TestCase):
    ...
    def test_filter_new_content(self):
        ...
        content_list = [
            ...
            {'url': 'https://example.com/page2', 'content': content2},  # 新
            {'url': 'https://example.com/page3', 'content': content1}   # 内容重复
        ]
...
class StressTest(unittest.TestCase):
    ...
    def test_incremental_crawler_large_dataset(self):
        ...
        contents = [f"Content {i}" for i in range(10000)]
        ...
        # 验证所有URL都已记录
        self.assertEqual(len(inc_crawler.url_status), 10000)
        self.assertEqual(len(inc_crawler.content_hashes), 10000)

        # 测试过滤性能
        start_time = time.time()
        new_urls = inc_crawler.filter_new_urls(urls[:1000])  # 过滤前1000个
        ...
        print(f"✅ 增量爬取器性能：10000个URL处理耗时 {elapsed:.4f} 秒，过滤耗时 {filter_elapsed:.4f} 秒")
